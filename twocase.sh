...
appid=$(java -jar vosp-api-wrappers-java-22.6.10.2.jar -action GetAppBuilds | grep VeraDemo | tail -n 2 | head -n 1 |awk '{print $2}' |sed 's/[^0-9]*//g')
set | grep sappid
...
echo $sandboxID
buildID=$(java -jar vosp-api-wrappers-java-22.6.10.2.jar -action GetBuildList -appid $appid -sandboxid $sandboxID | grep build_id | awk '{print $2}' | tail -n 1 | sed 's/[^0-9]*//g')
echo $buildID
set | grep buildID
